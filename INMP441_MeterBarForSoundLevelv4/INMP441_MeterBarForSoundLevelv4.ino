 ...
 * 
 * 
 * weeral compile errors config var niet gekend
 * snippet gevonden compileerd nu
 * nu pin configuratie

 hardware
controller heltec wifi kit 32 met oled display
i2s microfoon INMP441
voeding 3.3v no more!!!
core0 meting
core1 display dB 
...
#define MIC_REF_DB        87//94.0    // Value at which point sensitivity is specified in datasheet (dB)
...
//  };
  ...
  const i2s_config_t i2s_config = 
  {
     ...
     sample_rate : SAMPLE_RATE,               
     bits_per_sample :i2s_bits_per_sample_t(SAMPLE_BITS),                    // 16bit samples
     ...
     dma_buf_len : DMA_BANK_SIZE,                                            // samples per buffer
     use_apll: true,
     tx_desc_auto_clear: false,
     fixed_mclk: 0
  };
...
void setup() 
{
  ...
  xTaskCreatePinnedToCore(
    mic_i2s_reader_task,                 /* Task function. */
    "Mic I2S Reader",                    /* name of task. */
    I2S_TASK_STACK,                      /* Stack size of task */
    NULL,                                /* parameter of the task */
    I2S_TASK_PRI,                        /* priority of the task */
    &Task1,                              /* Task handle to keep track of created task */
    0);                                  /* Core */ //core 1 = altijd void loop
...
// Draw Decibel Meter
void drawDecibelMeter(double pre)
{  auto &display = *(Heltec.display);
   // clear the display
   display.clear();
   display.setTextAlignment(TEXT_ALIGN_LEFT);
   display.setFont(ArialMT_Plain_24);
   String message = String(pre) + " dB";
   display.drawString(0, 26, message);
   display.display(); // verdomme instinker
